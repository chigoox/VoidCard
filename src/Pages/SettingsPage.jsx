  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-5">
        <input className="rounded bg-slate-900 p-2" value={form.username || ''} onChange={(e) => update('username', e.target.value)} placeholder="username" />
        <input className="rounded bg-slate-900 p-2" value={form.name || ''} onChange={(e) => update('name', e.target.value)} placeholder="full name" />
        <input className="rounded bg-slate-900 p-2" value={form.title || ''} onChange={(e) => update('title', e.target.value)} placeholder="title" />
        <textarea className="rounded bg-slate-900 p-2" value={form.bio || ''} onChange={(e) => update('bio', e.target.value)} placeholder="bio" />

        <h3 className="font-semibold mt-2">Profile Links</h3>
        {(form.links || []).map((link, idx) => (
          <div key={`link-${idx}`} className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <input className="rounded bg-slate-900 p-2" value={link.label || ''} onChange={(e) => updateLink(idx, 'label', e.target.value)} placeholder="Button label" />
            <input className="rounded bg-slate-900 p-2" value={link.url || ''} onChange={(e) => updateLink(idx, 'url', e.target.value)} placeholder="https://..." />
          </div>
        ))}
      ...
      <div className="rounded-2xl border border-cyan-300/30 bg-cyan-400/10 p-6">
        ...
        <input className="mt-3 w-full rounded bg-slate-900 p-2" value={cardId} onChange={(e) => setCardId(e.target.value)} placeholder="Card ID (UID or serial)" />
